---
title: アイマスもくもく会2025年まとめ
date: "2025-12-20T00:01:00.000Z"
description: "2025年もアイマスもくもく会でしたよ！"
tags: ["imastudy", "imas_mokumoku"]
---

<img width="720" alt="header" src="/assets/images/posts/20241221-imas-mokumoku/header.png">

こんにちは、.ごっちです。

6年くらい アイマスもくもく会を運営しているのですが、今年もやめずに開催できました。過年の成果は以下の記事を見てください。

- アイマスもくもく会2024年まとめ - .ごっちの日記: https://blog.yougoto.dev/posts/20241221-imas-mokumoku/
- アイマスもくもく会2023年まとめ - .ごっちの日記: https://blog.yougoto.dev/posts/20231224-imas-mokumoku/
- アイマスもくもく会2022年まとめ - .ごっちの日記: https://blog.yougoto.dev/posts/20221216-imas-mokumoku/
- アイマスもくもく会2021年まとめ - .ごっちの日記: https://blog.yougoto.dev/posts/20211223-imas-mokumoku/

## 総括

11回の開催でした。登録人数が0人の回が複数ありますが、主催は参加していたので実質開催です。

- IM@Study-mokumoku - connpass: https://imastudy-mokumoku.connpass.com/

<img width="1058" alt="image.png (255.6 kB)" src="https://img.esa.io/uploads/production/attachments/10836/2025/12/18/43446/a3bd70bf-ff2d-4254-9470-99d598901e4e.png">

## 運営

去年と全く変わらずDiscordサーバーでもくもくしていました。フローも大きな変更をいれてません。

## 2025年のもくもく会の成果

2025年もたくさんの成果がありました。参加してくださったプロデューサーさん ありがとうございました！

- DiscordサーバのイベントをDeno.cronを使用し、自動で作成するようにする
- display: 283;の改修
    - https://display283.arrow2nd.com/
- Vision Proでライディング

![Untitled 12](https://hackmd.io/_uploads/HJ0PeK8Pkx.jpg)

- Vision Pro内にアクリルスタンドを立てる

![IMG_2803](https://hackmd.io/_uploads/r1-usvRple.jpg)

- 初星学園のロゴ再現のつづき
    - https://hatsuboshi-logo.pages.dev/

![GkO7sL0aoAA94f6](https://github.com/user-attachments/assets/01ee50f3-dfe8-4f6b-b27a-b64218cc986b)

- シンデレラガールズ声優さんの配役情報をRDF化してSPARQLで検索できるシステム
    - https://api.hhiro.net/rdf/?r=imascast.rdf
- imasparqlサーバをローカルで動かしてみる
- imasparql-mcpを作ってみる

<blockquote class="twitter-tweet"><p lang="ja" dir="ltr">成果: imasparqlのMCPサーバを動かした。 <a href="https://twitter.com/hashtag/imas_mokumoku?src=hash&amp;ref_src=twsrc%5Etfw">#imas_mokumoku</a> <a href="https://t.co/sUKRMOskNa">pic.twitter.com/sUKRMOskNa</a></p>&mdash; .ごっち (@gggooottto) <a href="https://twitter.com/gggooottto/status/1978817345826033671?ref_src=twsrc%5Etfw">October 16, 2025</a></blockquote> <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>

- langchain/langgraphからim@sparql呼び出す
